"""Unit tests for the Mexicano placeholder skeleton builder (issues #259, #260)."""
...
def test_odd_team_count_includes_a_bye_slot_every_round() -> None:
    """5 entrants: 4 play (adjacent pairs), 1 sits in the last slot every round."""
    skeleton = build_mexicano_skeleton(team_count=5, games_per_player=2)
    for round_skeleton in skeleton.rounds:
        assert round_skeleton.bye_slot == 4
        assert round_skeleton.matches == ((0, 1), (2, 3))


def test_odd_team_count_schedules_swiss_style_round_count() -> None:
    """Round count follows ceil(games_per_player * n / (n - 1)), same formula as Swiss."""
    # ceil(2 * 5 / 4) = ceil(2.5) = 3
    skeleton = build_mexicano_skeleton(team_count=5, games_per_player=2)
    assert skeleton.round_count == 3
