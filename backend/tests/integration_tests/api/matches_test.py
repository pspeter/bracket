...
async def test_update_match(
    ...
    async with (
        ...
        # Scores now live on match_sets; the match-level update only edits round/court/duration.
        body = {
            "round_id": round_inserted.id,
            "court_id": None,
        }
...
async def test_update_match_fails_when_stage_has_not_started(
    ...
    async with (
        ...
        # Starting a set (moving it to IN_PROGRESS) is refused while its stage is inactive.
